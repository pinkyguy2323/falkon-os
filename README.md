...
**Предустановлено:** ghostty + vscode + firefox + lutris + файловый менеджер (dolphin/thunar) + vlc + gimp + AI Claude Code и Codex. Тяжёлое (discord, telegram, steam, obs, офис) ставится в 1 клик через `falkon-welcome` — в ISO не влезло бы в лимит релиза 2 ГБ.
...
3. **Пользователь, пароль, оформление (KDE/Hypr/консоль), язык.** Дальше 10–20 минут всё само: оба графических стека ставятся сразу, выбранное оформление включается по умолчанию.
